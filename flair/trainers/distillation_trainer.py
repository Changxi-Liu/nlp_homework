...
class ModelDistiller(ModelTrainer):
	...
	def final_test(
		...
		log_line(log)
		print('\x1b[6;30;42m'  'OutPut: ' + '\x1b[0m', __file__, sys._getframe(0).f_lineno,sys._getframe(1).f_in) 
		...
		print(self.model,type(self.corpus) )
		if type(self.corpus) is MultiCorpus:
			for subcorpus in self.corpus.corpora:
				log_line(log)
				log.info('current corpus: '+subcorpus.name)
	...
	def gpu_friendly_assign_embedding(self,loaders, selection = None,gen=False):
		# pdb.set_trace()
