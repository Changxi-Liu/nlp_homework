...
class ModelFinetuner(ModelDistiller):
	...
	gen=False):
		...
		print('\x1b[6;30;42m'  'OutPut: ' + '\x1b[0m', __file__, sys._getframe(0).f_lineno,sys._getframe(1).f_code.co_name)
		...
		if overall_test:
			print('\x1b[6;30;42m'  'OutPut: ' + '\x1b[0m', __file__, sys._getframe(0).f_lineno,sys._getframe(1).f_code.co_name,self.corpus.test)
			if not gen:
				...
				loader=ColumnDataLoader(list(self.corpus.train),eval_mini_batch_size, use_bert=self.use_bert,tokenizer=self.bert_tokenizer, model = self.model, sentence_level_batch = self.sentence_level_batch, sort_data=sort_data)
			loader.assign_tags(self.model.tag_type,self.model.tag_dictionary)
