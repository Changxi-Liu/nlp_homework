...
class ColumnCorpus(Corpus):
    def __init__(
        ...
        train = ColumnDataset(
            ...
            in_memory=in_memory,
            target_scheme=target_scheme_train,
        )
        ...
        if test_file is not None:
            test = ColumnDataset(
                ...
                comment_symbol=comment_symbol,
                in_memory=in_memory,
                target_scheme=target_scheme_test,
            )
        else:
        ...
        if dev_file is not None:
            dev = ColumnDataset(
                ...
                comment_symbol=comment_symbol,
                in_memory=in_memory,
                target_scheme=target_scheme_dev,
            )
        else:
            train_length = len(train)
                        ...
                        if self.in_memory:
                            if self.tag_to_bioes is not None:
                                sentence.convert_tag_scheme(
                                    tag_type=self.tag_to_bioes, target_scheme=target_scheme
                                )
                            #print('\x1b[6;30;42m'  'OutPut: ' + '\x1b[0m', __file__, sys._getframe(0).f_lineno,fields,sentence) 
#                            new_sentence: Sentence = Sentence()
#                            for token in sentence.tokens:
#                                new_sentence.add_token(token)
#                           
##                            token_tmp = self.str2token('<EOS> B-X')
##                            token_tmp.add_tag(  )
##                            new_sentence.add_token(token_tmp)
#                            for token in sentence.tokens:
#                                if token.text == '<EOS>':
#                                    break
#                                else:
#                                    new_sentence.add_token(copy.deepcopy(token))
                            #print('\x1b[6;30;42m'  'OutPut: ' + '\x1b[0m', __file__, sys._getframe(0).f_lineno,fields,new_sentence) 
