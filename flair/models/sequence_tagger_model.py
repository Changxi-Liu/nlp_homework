...
class SequenceTagger(flair.nn.Model):
        def __init__(
                ...
                if self.use_rnn:
                        ...
                        #self.linear1 = torch.nn.Linear( self.embeddings.embedding_length, self.hidden_size )
                        #self.linear2 = torch.nn.Linear( self.hidden_size,len(tag_dictionary) )
                       self.linear = torch.nn.Linear(
        ...
        def evaluate(
                ...
                print('\x1b[6;30;42m'  'OutPut: ' + '\x1b[0m', __file__, sys._getframe(0).f_lineno) 
...
#               self.hidden1 = torch.nn.Linear( self., self.hidden_size)
                features = self.linear(sentence_tensor)
                #features1 = self.linear1(sentence_tensor)
                #features2 = F.relu( features1)
                #features  = self.linear2(features2)
        ...
        def _obtain_labels(
                self, feature, sentences, get_all_tags: bool = False,gen=False
                ...
                #print('\x1b[6;30;42m'  'OutPut: ' + '\x1b[0m', __file__, sys._getframe(0).f_lineno, feature.shape)
                if self.predict_posterior:
                        
                        lengths1 = torch.Tensor(lengths).long()
                 #       print('\x1b[6;30;42m'  'OutPut: ' + '\x1b[0m', __file__, sys._getframe(0).f_lineno, feature.shape,lengths1)
                ...
                for i, vals in enumerate(zip(feature, lengths)):
                        ...
                        if self.use_crf and not self.predict_posterior:
                                # pdb.set_trace()
                                #print('\x1b[6;30;42m'  'OutPut: ' + '\x1b[0m', __file__, sys._getframe(0).f_lineno,self.remove_x)
                                if self.remove_x:
                                        
                                        #print('\x1b[6;30;42m'  'OutPut: ' + '\x1b[0m', __file__, sys._getframe(0).f_lineno, current_mask)
                                        #print('\x1b[6;30;42m'  'OutPut: ' + '\x1b[0m', __file__, sys._getframe(0).f_lineno, feats.shape,length)
                                        if gen:
                                                current_feat = feats
                                        else:
                                        ...
                                        #print('\x1b[6;30;42m'  'OutPut: ' + '\x1b[0m', __file__, sys._getframe(0).f_lineno, current_feat.shape,length)
                                        confidences, tag_seq, scores = self._viterbi_decode(current_feat, all_scores=get_all_tags, current_idx = i,)
                                        #print('\x1b[6;30;42m'  'OutPut: ' + '\x1b[0m', __file__, sys._getframe(0).f_lineno, len(tag_seq),length)
...
class FastSequenceTagger(SequenceTagger):
        ...
        def evaluate(
                ...
                with torch.no_grad():
                        ...
                        lines: List[str] = []
                        print('\x1b[6;30;42m'  'OutPut: ' + '\x1b[0m', __file__, sys._getframe(0).f_lineno,out_path)
                        if out_path is not None:
                        ...
                        for batch in data_loader:
                                ...
                                with torch.no_grad():
                                        ...
                                        tags, _ = self._obtain_labels(features, batch,gen=gen)
                                if not speed_test:
                                        eval_loss += loss
                                        ...
                                        for (sentence, sent_tags) in zip(batch, tags):
                                                #print(sentence)
                                                #idx = 0
                                                #gold_tags = [tag.tag for tag in sentence.get_spans(self.tag_type)]
                                                need2predict = False
                                                for (token, tag) in zip(sentence.tokens, sent_tags):
                                                        token: Token = token
                                                        token.add_tag_label("predicted", tag)
                                                        ...
                                                        if gen:
                                                                ...
                                                                #original_tag_str =
                                                                if need2predict:
                                                                        tag_value_tmp = tag.value
                                                                elif  text_tmp== '<EOS>':
                                                                        need2predict = True
                                                                        tag_value_tmp =  token.get_tag(self.tag_type).value
                                                                else:
                                                                        tag_value_tmp =  token.get_tag(self.tag_type).value
                                                                #else:
